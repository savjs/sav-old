...
import {pascalCase, hyphenCase, prop, isString} from 'sav-util'
...
async function writeVueFile (dir, modalName, modal, opts) {
  ...
  let modalComponent = {
    component: modalName,
    path: isString(modal.path) ? modal.path : hyphenCase(modalName),
    children: []
  }
  ...
  for (let routeName in modal.routes) {
    if (isClientRouter(modal, modal.routes[routeName])) {
      ...
      let routeComponent = {
        ...
        path: isString(route.path) ? route.path : hyphenCase(routeName)
      }
