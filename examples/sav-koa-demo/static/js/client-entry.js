...
function queueWatcher (watcher) {
  ...
  if (has[id] == null) {
    ...
    if (!flushing) {
      ...
      queue.splice(Math.max(i, index$1$1) + 1, 0, watcher);
    }
    // queue the flush
    if (!waiting) {
      waiting = true;
...
setTimeout(function () {
  ...
  if ("production" !== 'production' &&
      ...
      inBrowser$1 && typeof console !== 'undefined') {
    console[console.info ? 'info' : 'log'](
...
function toStringType$1(val) {
  return Object.prototype.toString.call(val).slice(8, -1);
}

function isObject$2(arg) {
  return toStringType$1(arg) === 'Object' && arg !== null;
}
...
let probe$1 = {
  Map: defined$1(typeof Map),
  Proxy: defined$1(typeof Proxy),
  MessageChannel: defined$1(typeof MessageChannel),
  localStorage: defined$1(typeof localStorage),
  XMLHttpRequest: defined$1(typeof XMLHttpRequest),
  MutationObserver: defined$1(typeof MutationObserver),
  FormData: defined$1(typeof FormData),
  window: defined$1(typeof window),
  document: defined$1(typeof document)
};
...
// jsuri https://code.google.com/r/jonhwendell-jsuri/
// https://username:password@www.test.com:8080/path/index.html?this=that&some=thing#content
const REKeys$1 = ['source', 'protocol', 'authority', 'userInfo', 'user', 'password', 'host', 'port', 'relative', 'path', 'directory', 'file', 'query', 'anchor'];
const URL_RE$1 = /^(?:(?![^:@]+:[^:@/]*@)([^:/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#/]*\.[^?#/.]+(?:[?#]|$)))*\/?)?([^?#/]*))(?:\?([^#]*))?(?:#(.*))?)/;

function parseUrl$1(str) {
  let _uri = {};
  let _m = URL_RE$1.exec(str || '');
  let _i = REKeys$1.length;
  while (_i--) {
    _uri[REKeys$1[_i]] = _m[_i] || '';
  }
  return _uri;
}

function stringifyUrl$1(uri) {
  let str = '';
  if (uri) {
    if (uri.host) {
...
const _encode$1 = encodeURIComponent;
const r20$1 = /%20/g;
const rbracket$1 = /\[]$/;

function buildParams$1(prefix, obj, add) {
  if (Array.isArray(obj)) {
    // Serialize array item.
    obj.forEach(function (v, i) {
      if (rbracket$1.test(prefix)) {
        // Treat each array item as a scalar.
        add(prefix, v);
      } else {
        // Item is non-scalar (array or object), encode its numeric index.
        buildParams$1(prefix + '[' + (typeof v === 'object' ? i : '') + ']', v, add);
      }
    });
  } else if (isObject$2(obj)) {
    // Serialize object item.
    for (let name in obj) {
      buildParams$1(prefix + '[' + name + ']', obj[name], add);
    }
  } else {
...
function parseQuery$2(str, opts = {}) {
  let _querys = {};
...
function stringifyQuery$2(query) {
  ...
  let _add = (key, value) => {
    /* jshint eqnull:true */
    _str.push(_encode$1(key) + '=' + (value === null || value === undefined ? '' : _encode$1(value)));
    // _str.push(( key ) + "=" +  (value == null ? "" : ( value )));
  };
  let _str = [];
  query || (query = {});
  for (let x in query) {
    buildParams$1(x, query[x], _add);
  }
  return _str.join('&').replace(r20$1, '+');
}

let PROMISE$1 = Promise;
let promise$1 = {
  resolve: PROMISE$1.resolve.bind(PROMISE$1),
  reject: PROMISE$1.reject.bind(PROMISE$1),
  all: PROMISE$1.all.bind(PROMISE$1),
  then: (fn, reject) => {
    return new PROMISE$1(fn, reject);
  }
};
...
function ajax$1(opts, next) {
  ...
  if (isGet) {
    if (opts.data) {
      let u = parseUrl$1(opts.url);
      let q = parseQuery$2(u.query);
      for (let x in opts.data) {
        q[x] = opts.data[x];
      }
      u.query = stringifyQuery$2(q);
      opts.url = stringifyUrl$1(u);
      opts.data = null;
    }
  } else if (isPost) {
    data = opts.data;
  ...
  try {
    ...
    if (opts.headerOnly) {
      req.onreadystatechange = function () {
        ...
        if (req.readyState === 2) {
          // HEADERS_RECEIVED
          let headers = parseHeaders$1(req.getAllResponseHeaders(), opts.camelHeaders);
          req.abort();
    ...
    req.onload = function () {
      ...
      if ([200, 304, 206, 0].indexOf(req.status) === -1) {
        ...
        emit(null, data, parseHeaders$1(req.getAllResponseHeaders(), opts.camelHeaders));
      }
    };
...
function parseHeaders$1(str, camelHeaders) {
  let ret = {};
  str.trim().split('\n').forEach(function (key) {
    key = key.replace(/\r/g, '');
    let arr = key.split(': ');
    let name = arr.shift().toLowerCase();
    ret[camelHeaders ? camelCase$1(name) : name] = arr.shift();
  });
  return ret;
}
...
var SavForm = {
  render: function () {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('form', { attrs: { "action": _vm.location.path }, on: { "submit": function ($event) {
          $event.preventDefault();_vm.submit($event);
        } } }, [_vm._t("default")], 2);
  },
  staticRenderFns: [],
  props: {
  ...
  methods: Object.assign({}, mapActions(['replaceHistoryState']), {
    submit($event) {
...
var App = {
  render: function () {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { attrs: { "id": "app" } }, [_c('ul', [_c('router-link', { attrs: { "to": { name: 'HomeIndex' } } }, [_vm._v("home")]), _vm._v(" "), _c('router-link', { attrs: { "to": { name: 'HomeAbout' } } }, [_vm._v("about")]), _vm._v(" "), _c('router-link', { attrs: { "to": { name: 'AccountLogin' } } }, [_vm._v("login")]), _vm._v(" "), _c('router-link', { attrs: { "to": { name: 'AccountRegister' } } }, [_vm._v("register")]), _vm._v(" "), _c('router-link', { attrs: { "to": { name: 'ArticlePosts' } } }, [_vm._v("articles")])], 1), _vm._v(" "), _c('router-view', { staticClass: "view" })], 1);
  },
  staticRenderFns: []
};
...
Vue$3.use(VueRouter);
Vue$3.use(FluxVue);
Vue$3.use(components);

var bootstrap = function ({ routes }) {
