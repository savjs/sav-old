...
function set(target, key, val) {
  ...
  if (target._isVue || ob && ob.vmCount) {
    undefined !== 'production' && warn$1('Avoid adding reactive properties to a Vue instance or its root $data ' + 'at runtime - declare it upfront in the data option.');
    return val;
  }
  if (!ob) {
    target[key] = val;
...
function del(target, key) {
  ...
  var ob = target.__ob__;
  if (target._isVue || ob && ob.vmCount) {
    undefined !== 'production' && warn$1('Avoid deleting properties on a Vue instance or its root $data ' + '- just set it to null.');
    return;
  }
  if (!hasOwn(target, key)) {
    return;
...
strats.data = function (parentVal, childVal, vm) {
  if (!vm) {
    ...
    if (typeof childVal !== 'function') {
      undefined !== 'production' && warn$1('The "data" option should be a function ' + 'that returns a per-instance value in component ' + 'definitions.', vm);
      return parentVal;
    }
    if (!parentVal) {
      return childVal;
...
function checkComponents(options) {
  for (var key in options.components) {
    ...
    if (isBuiltInTag(lower) || config.isReservedTag(lower)) {
      warn$1('Do not use built-in or reserved HTML elements as component ' + 'id: ' + key);
    }
  }
}
...
function assertProp(prop, name, value, vm, absent) {
  ...
  if (!valid) {
    warn$1('Invalid prop: type check failed for prop "' + name + '".' + ' Expected ' + expectedTypes.map(capitalize).join(', ') + ', got ' + Object.prototype.toString.call(value).slice(8, -1) + '.', vm);
    return;
  }
...
if (undefined !== 'production') {
  ...
  var warnNonPresent = function (target, key) {
    warn$1("Property or method \"" + key + "\" is not defined on the instance but " + "referenced during render. Make sure to declare reactive data " + "properties in the data option.", target);
  };
...
function updateListeners(on, oldOn, add, remove$$1, vm) {
  ...
  for (name in on) {
    ...
    if (isUndef(cur)) {
      undefined !== 'production' && warn$1("Invalid handler for event \"" + event.name + "\": got " + String(cur), vm);
    } else if (isUndef(old)) {
      if (isUndef(cur.fns)) {
...
function resolveAsyncComponent(factory, baseCtor, context) {
  ...
  if (isDef(factory.contexts)) {
    ...
    var reject = once$1(function (reason) {
      undefined !== 'production' && warn$1("Failed to resolve async component: " + String(factory) + (reason ? "\nReason: " + reason : ''));
...
var Watcher = function Watcher(vm, expOrFn, cb, options) {
  ...
  if (typeof expOrFn === 'function') {
    ...
    if (!this.getter) {
      ...
      undefined !== 'production' && warn$1("Failed watching path: \"" + expOrFn + "\" " + 'Watcher only accepts simple dot-delimited paths. ' + 'For full control, use a function instead.', vm);
    }
  }
  ...
  var loop = function (key) {
    ...
    if (undefined !== 'production') {
      if (isReservedProp[key] || config.isReservedAttr(key)) {
        warn$1("\"" + key + "\" is a reserved attribute and cannot be used as component prop.", vm);
      }
...
function initData(vm) {
  ...
  if (!isPlainObject(data)) {
    data = {};
    undefined !== 'production' && warn$1('data functions should return an object:\n' + 'https://vuejs.org/v2/guide/components.html#data-Must-Be-a-Function', vm);
  }
  ...
  while (i--) {
    if (props && hasOwn(props, keys[i])) {
      undefined !== 'production' && warn$1("The data property \"" + keys[i] + "\" is already declared as a prop. " + "Use prop default value instead.", vm);
...
function initComputed(vm, computed) {
  ...
  for (var key in computed) {
    ...
    if (undefined !== 'production') {
      if (getter === undefined) {
        warn$1("No getter function has been defined for computed property \"" + key + "\".", vm);
        getter = noop;
      }
    }
...
function initMethods(vm, methods) {
  ...
  for (var key in methods) {
    ...
    if (undefined !== 'production') {
      if (methods[key] == null) {
        warn$1("method \"" + key + "\" has an undefined value in the component definition. " + "Did you reference the function correctly?", vm);
      }
...
function _createElement(context, tag, data, children, normalizationType) {
  if (isDef(data) && isDef(data.__ob__)) {
    undefined !== 'production' && warn$1("Avoid using observed data object as vnode data: " + JSON.stringify(data) + "\n" + 'Always create fresh vnode data objects in each render!', context);
...
function bindObjectProps(data, tag, value, asProp) {
  if (value) {
    if (!isObject(value)) {
      undefined !== 'production' && warn$1('v-bind without argument expects an Object or Array value', this);
    } else {
      if (Array.isArray(value)) {
...
function Vue$3(options) {
  if (undefined !== 'production' && !(this instanceof Vue$3)) {
  ...
  Vue.extend = function (extendOptions) {
    ...
    if (undefined !== 'production') {
      if (!/^[a-zA-Z][\w-]*$/.test(name)) {
        warn$1('Invalid component name: "' + name + '". Component names ' + 'can only contain alphanumeric characters and the hyphen, ' + 'and must start with a letter.');
      }
    }
      ...
      if (!definition) {
        ...
        if (undefined !== 'production') {
          if (type === 'component' && config.isReservedTag(id)) {
            warn$1('Do not use built-in or reserved HTML elements as component ' + 'id: ' + id);
          }
        }
...
function query(el) {
  if (typeof el === 'string') {
    ...
    if (!selected) {
      undefined !== 'production' && warn$1('Cannot find element: ' + el);
      ...
      if (isUndef(oldStartVnode)) {
        ...
        if (isUndef(idxInOld)) {
          ...
          if (undefined !== 'production' && !elmToMove) {
            warn$1('It seems there are duplicate keys that is causing an update error. ' + 'Make sure each v-for item has a unique key.');
          }
      ...
      if (isDef(children)) {
        ...
        if (!elm.hasChildNodes()) {
          ...
          if (!childrenMatch || childNode) {
            if (undefined !== 'production' && typeof console !== 'undefined' && !bailed) {
              bailed = true;
...
function setSelected(el, binding, vm) {
  ...
  if (isMultiple && !Array.isArray(value)) {
    undefined !== 'production' && warn$1("<select multiple v-model=\"" + binding.expression + "\"> " + "expects an Array value for its binding, but got " + Object.prototype.toString.call(value).slice(8, -1), vm);
    return;
  }
  var selected, option;
...
Vue$3.prototype.__patch__ = inBrowser$1 ? patch : noop;
...
setTimeout(function () {
  ...
  if (undefined !== 'production' && config.productionTip !== false && inBrowser$1 && typeof console !== 'undefined') {
...
function initDispatch (ref) {
  ...
  prop$$1('dispatch', proxyApi(dispatch));
}
...
var HomeIndex = {
  render: function () {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { staticClass: "home-index" }, [_vm._v("\n  HomeIndex-" + _vm._s(_vm.welcome) + "\n")]);
  },
  staticRenderFns: [],
  name: 'HomeIndex',
  getters: ['welcome'],
...
var HomeAbout = {
  render: function () {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { staticClass: "home-about" }, [_vm._v("\n  HomeAbout\n")]);
  },
  staticRenderFns: [],
  name: 'HomeAbout',
  getters: [],
...
var savSchema = createCommonjsModule$$1(function (module, exports) {
  /*!
   * sav-schema v0.0.17
   * (c) 2017 jetiny 86287344@qq.com
   * Release under the MIT License.
...
var SavSchema = unwrapExports$$1(savSchema);
...
function createRoute(ref) {
  if (!ref.route) {
    ...
    prop(savRoute, 'regexp', index.compile(path));
...
function promiseNext() {
  let promise$$1 = Promise.resolve();
  let ret = (resolve, reject) => {
    if (resolve || reject) {
      promise$$1 = promise$$1.then(resolve, reject);
    }
    return promise$$1;
  };
  return ret;
...
function fetchRoute(opts) {
  ...
  try {
    ...
    ctx.fetch = ajaxFetch;
    this.emit('fetch', ctx, next$$1); // prepare
    next$$1(() => {
...
var index$7 = Object.assign({}, Home$1);
...
var index$11 = Object.assign({}, User$2);
...
savClient_1({ contract: index$2, flux, router }).then(() => {
  ...
  let vm = new Vue$3(Object.assign({
    vaf,
    router
  }, App));
  let app = {
